...
**\*\*Question: Create the transition matrices below. Enter `C`for the probability of remaining in the same state between model states, e.g. H -\> H, as it equals 1- the sum of all other probabilities in the same row.**
...
**\*\*Question: What is a PSA? (Hint: try asking ChatGPT. Is it right?). What type of model uncertainty does it address? What advantages does it have over one-way sensitivity analysis?**

**\*\*Question: One of the most challenging parts of doing a PSA is defining distributions for each parameter using the available evidence. Often this requires summary information from the literature (e.g. the parameter mean and sd), into the parameters required for your desired distribution. We will do this the utility parameters as an example. Given the following means and standard deviations, determine the shape 1 and shape 2 parameters in a beta distribution.**
...
**\*\*Question: Interpret the CE plane. What does the black line represent? Is treatment A cost-effective at a WTP of \$50,000/QALY? Does this information change your decision on whether to fund treatment A?**
