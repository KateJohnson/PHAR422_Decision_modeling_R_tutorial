...
![](Figures/Transition_matrix.png){width="320"}

**Hint: Enter `C` for the probability of remaining in the same state between model cycles (e.g. H -\> H). It equals 1- the sum of all other probabilities in the same row.**
...
**\*\*Question: Add in the costs of treatment `c_trtA`** **to one of the** `cost=` **functions. Which states should it be added to?\*\***
...
-   Sicker utility `u_S2`: mean= 0.5, sd=0.0329

-   Utility of sick state on treatment A `u_trtA`: mean=0.95, sd= 0.0329
...
estBetaParams(, ) # round to the nearest integer
```
